...
(:
 ...
 :)
declare variable $config:default-template :="wvh-schriften.html";
...
declare variable $config:facets := [
  ...
  "hierarchical": true()
    ...
    map {
  "dimension": "place",
  "heading": "Entstehungsort",
  "max": 100,
  "hierarchical": false(),
  "output": function($label) {
    let $doc := doc("/db/apps/wvh-schriften/resources/registers/places.xml")
    let $entry := $doc//tei:place[@xml:id = $label]
    let $name := normalize-space($entry/tei:placeName[@type='main'][1])
    return if ($name) then $name else $label
  }
},
...
declare variable $config:data-exclude :=
    ...
    doc($config:data-root || "/about.xml")//tei:text
;
declare variable $config:register-person := doc($config:data-root || "/db/apps/wvh-schriften/resources/registers/person.xml");
declare variable $config:register-place := doc($config:data-root || "/db/apps/wvh-schriften/resources/registers/places.xml");
declare variable $config:register-organization := doc($config:data-root || "/db/apps/wvh-schriften/resources/registers/organizations.xml");
declare variable $config:register-keyword := doc($config:data-root || "/db/apps/wvh-schriften/resources/registers/keywords.xml");
(:~
 : The main ODD to be used by default
 :)
declare variable $config:default-odd :="wvh-schriften.odd";
...
(:~
 ...
 :)
declare variable $config:odd-available :=("wvh-schriften.odd");
